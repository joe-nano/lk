...
enum { ID_CODE = wxID_HIGHEST+149, ID_SAVE, ID_PARSE, ID_IBRK, ID_ASM, ID_BYTECODE, ID_OUTPUT,
	ID_LOAD, ID_RESET, ID_STEP_ASM, ID_DEBUG, ID_STEP_LINE, ID_EVAL, ID_CALL };
...
public:
	...
	VMTestFrame() : wxFrame( NULL, wxID_ANY, "LK-VM", wxDefaultPosition, wxSize(1200,900) )
	{
		...
		buttons->Add( new wxButton( this, ID_CALL, "call(...)"), 0, wxALL|wxEXPAND, 3 );
	...
	void OnCommand( wxCommandEvent &evt )
	{
		...
		case ID_CALL:
		{
			static wxString func_call;
			func_call = wxGetTextFromUser("Function name:", "Query", func_call );

			wxArrayString parts = wxStringTokenize( func_call, " \t\r\n()" );
			...
			bool ok = false;
			int dpos = parts[0].Find('.');
			if ( dpos == wxNOT_FOUND )
			{			
				ok = vm.call( parts[0], args, result, lk::vm::NORMAL );
			}
			else if ( lk::env_t *e = vm.get_env() )
			{
				wxString table( parts[0].Mid(0,dpos) );
				wxString field( parts[0].Mid(dpos+1) );

				int faddr = -1;
				if ( lk::vardata_t *ptable = e->lookup( table, true ) )
					if ( ptable->type() == lk::vardata_t::HASH )
						if ( lk::vardata_t *pfield = ptable->lookup( field ) )
							if ( pfield->type() == lk::vardata_t::INTFUNC )
								faddr = pfield->faddr();


				if ( faddr >= 0 )
				{
					lk_string name( table + "{" + field + "}" );
					ok = vm.call( (size_t)faddr, args, result, &name, lk::vm::NORMAL );
				}
				else
					m_output->AppendText( "could not locate " + parts[0] + "\n" );
			}
			else
				m_output->AppendText( "could not interpret call name structure\n" );

			m_output->AppendText( "CALL " + wxJoin(parts,' ')  + ": " + wxString(ok?"ok":"fail")  + "\n" );
			if ( !ok )
				m_output->AppendText( vm.error() + "\n" );
			else 
				m_output->AppendText( "\tresult: " + result.as_string() + "\n");
