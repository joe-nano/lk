...
lk::env_t *vm::get_env()
{
	return (frames.size()==0) ? 0 : &frames.back()->env;
}
...
bool vm::call( size_t iaddr,
	std::vector< vardata_t > &args,
	vardata_t &result,
	const lk_string *callname,
	ExecMode mode )
{
	// save VM state if need to roll back
	size_t nf_save = frames.size();
	size_t sp_save = sp;


	// simulate a CALL instruction and create a new frame
	frames.push_back( new frame( &frames.back()->env, sp+args.size()+2, bc->program.size(), args.size() ) );
	frame &F = *frames.back();
	F.id =  callname ? *callname : "vm::call(...)";	
		
	
	// create return value space on stack, referencing the passed result parameter
	sp++;

	vardata_t *__args = new vardata_t;
	__args->empty_vector();
		
	// push the arguments onto the stack and create the __args implicit variable in the Frame
	for( size_t i=0;i<args.size();i++ )
	{
		vardata_t argref;
		argref.assign( &( args[i]) );

		__args->vec()->push_back( argref );
		stack[sp++].copy( argref );
	}
						
	F.env.assign( "__args", __args );
	
	// alloc stack space for function address as would be in normal CALL
	sp++;
			
	// run code starting at requested instruction pointer
	ip = iaddr;
	bool ok = run( mode );

	if ( sp > sp_save && sp >= 1 )
		result.copy( stack[sp-1] );
	else
		result.nullify();

	// in case of error inside function call, 
	// restore the old stack pointer and
	// delete any left over frames
	for( size_t i=sp_save; i < sp; i++ )
		stack[i].nullify();

	sp = sp_save;
	

	while( frames.size() > nf_save )
	{
		delete frames.back();
		frames.pop_back();
	}	
...
bool vm::call( const lk_string &function,
	std::vector< vardata_t > &args,
	vardata_t &result, 
	ExecMode mode )
{	
	vardata_t *v = get_env()->lookup( function, true );
	if ( !v || v->type() != lk::vardata_t::INTFUNC )
		return error( "vm::call --> no function named '%s' found in environment", (const char*)function.c_str() );

	return call( v->faddr(), args, result, &function, mode );
}
