...
{
	LK_DOC("strpos", "Locates the first instance of a character or substring in a string, case sensitive.", "(string, string):integer");
	lk_string s = cxt.arg(0).as_string();
...
static void _stripos( lk::invoke_t &cxt )
{
	LK_DOC("stripos", "Locates the first instance of a character or substring in a string, case insensitive.", "(string, string):integer");
	lk_string s = lk::lower_case( cxt.arg(0).as_string() );
	lk_string f = lk::lower_case( cxt.arg(1).as_string() );
	lk_string::size_type idx = s.find(f);
	cxt.result().assign( (idx!=lk_string::npos) ? (int)idx : -1 );
}

static void _strfind( lk::invoke_t &cxt )
{
	LK_DOC( "strfind", "Returns whether a string exists within another, case sensitive.", "(string:source, string:query):boolean" );
	lk_string s = cxt.arg(0).as_string();
	lk_string f = cxt.arg(1).as_string();
	lk_string::size_type idx = s.find(f);
	cxt.result().assign( (idx!=lk_string::npos) ? 1.0 : 0.0 );
}

static void _strifind( lk::invoke_t &cxt )
{
	LK_DOC( "strifind", "Returns whether a string exists within another, case insensitive.", "(string:source, string:query):boolean" );
	lk_string s = lk::lower_case( cxt.arg(0).as_string() );
	lk_string f = lk::lower_case( cxt.arg(1).as_string() );
	lk_string::size_type idx = s.find(f);
	cxt.result().assign( (idx!=lk_string::npos) ? 1.0 : 0.0 );
